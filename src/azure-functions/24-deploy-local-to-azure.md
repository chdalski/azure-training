...
Now that we've created our Azure Resources it's time to deploy our local Function App to it's Azure counterpart.
...
From our local tests we also knew that our functions are located in the `api` subdirectory followed by the function name.
...
- Lookup the function name, as done above, and replace `<function-app-name>` in the url with it
